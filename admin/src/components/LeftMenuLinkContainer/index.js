...
//TODO toranova: dev admin-menu
// comment out && plugin.id !== "content-type-builder"
// uncomment 'general menu'
...
// Generate the list of plugin links (plugins without a mainComponent should not appear in the left menu)
  const pluginsLinks = Object.values(plugins)
    .filter(
      ...
      && plugin.id !== "content-type-builder"
    )
    .map(plugin => {
  ...
  const menu = {
    ...
    //uncomment the following to enable the general menu
    //general: {
    //  searchable: false,
    //  name: 'general',
    //  links: [
